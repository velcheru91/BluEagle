...
void BSP_UART1_Init()
{// Configuring uart1 PC4 (pin 37), PC5 (pin 38)
 SYSCTL_RCGCUART_R |= SYSCTL_PRUART_R1;  // activate UART1
  while((SYSCTL_PRUART_R & SYSCTL_PRUART_R1) == 0){}; // allow UART3 clock to stabilize
  SYSCTL_RCGCGPIO_R |= SYSCTL_RCGCGPIO_R2; // activate port C
  while((SYSCTL_PRGPIO_R & SYSCTL_RCGCGPIO_R2) == 0){}; // allow GPIO C clock to stabilize
  UART1_CTL_R &= ~UART_CTL_UARTEN;	// disable UART
  UART1_IBRD_R = 27; 	// IBRD = int(50,000,000/(16*115,200)) = int(27.1267361)
  UART1_FBRD_R = 8; 	// FBRD = round(0.1267361 * 64) = 8
  UART1_LCRH_R = 0x00000070;  // 8 bit, no parity bits, one stop, FIFOs
  UART1_CTL_R |= UART_CTL_TXE | UART_CTL_RXE | UART_CTL_UARTEN;	 // enable UART
